...
class WorldModel(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.latent_separate:
            cfg.h_obs_shape = {"state": (cfg.h_obs_vel_dim,)}
            self.h_obs_pos_dim = cfg.h_obs_vel_dim
            ...
            self._h_obs_e = self._h_obs_s + cfg.h_obs_vel_dim + cfg.h_obs_vel_dim
            self._latent_r_s = 0
            self._latent_r_e = self.cfg.latent_r_dim
            self._latent_h_s = self._latent_r_e
            self._latent_h_e = self._latent_h_s + self.h_obs_pos_dim
            self.cfg.latent_dim = self.cfg.latent_r_dim + self.h_obs_pos_dim # + cfg.h_obs_vel_dim
        else:
        ...
        self._reward = layers.mlp(
            self.cfg.latent_dim + cfg.action_dim,
            2 * [cfg.mlp_dim],
            max(cfg.num_bins, 1),
        )
        self._termination = (
            layers.mlp(self.cfg.latent_dim, 2 *
    ...
    def slice_h_observation(self, h_obs):
        ...
        """
        h_obs_pos = h_obs[..., 0:self.cfg.h_obs_vel_dim]
        h_obs_vel = h_obs[..., self.cfg.h_obs_vel_dim:
                        self.cfg.h_obs_vel_dim + self.cfg.h_obs_vel_dim]
    ...
    def encode(self, obs):
        ...
        if self.cfg.latent_separate:
            ...
            return torch.cat([z_r, h_obs_pos], dim=-1) , h_obs_vel
        if self.cfg.obs == "rgb" and obs.ndim == 5:
            return torch.stack([self._encoder[self.cfg.obs](o) for o in obs])
        return self._encoder[self.cfg.obs](obs)

    def next(self, z, a, h_obs_vel):
        """
