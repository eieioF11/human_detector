...
class WorldModel(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.latent_separate:
            ...
            self._reward_r = layers.mlp(
                cfg.latent_r_dim + cfg.action_dim,
                2 * [cfg.mlp_dim],
                max(cfg.num_bins, 1),
            )
            self._reward_h = layers.mlp(
                cfg.latent_h_dim + cfg.action_dim,
                2 * [cfg.mlp_dim],
                max(cfg.num_bins, 1),
            )
            self._reward = nn.ModuleDict(
                {
                    "robot": self._reward_r,
                    "human": self._reward_h,
                }
            )
            self._r_obs_s = 0
        ...
        if cfg.latent_separate:
            init.zero_(
                [
                    self._reward_r[-1].weight,
                    self._reward_h[-1].weight,
    ...
    def reward(self, z, a, h_only=False, r_only=False):
        """
        Predicts instantaneous (single-step) reward.
        """
        if self.cfg.latent_separate:
            z_r, z_h = self.slice_latent(z)
            z_next_r = self._reward["robot"](torch.cat([z_r, a], dim=-1))
            z_next_h = self._reward["human"](torch.cat([z_h, a], dim=-1))
            if h_only:
                return z_next_h
            if r_only:
                return z_next_r
            return z_next_r + z_next_h
        z = torch.cat([z, a], dim=-1)
