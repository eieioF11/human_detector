...
def rnn(in_dim, rnn_dim, out_dim, act=None):
    """
    RNN with LayerNorm and Mish activations.
    """
    return nn.Sequential(
        RNN(in_dim, rnn_dim),
        NormedLinear(rnn_dim, out_dim, act=act)
        if act
        else nn.Linear(rnn_dim, out_dim)
    )

def lstm(in_dim, lstm_dim, out_dim, act=None):
    """
    LSTM with LayerNorm and Mish activations.
    """
    return nn.Sequential(
        LSTM(in_dim, lstm_dim),
        NormedLinear(lstm_dim, out_dim, act=act)
        if act
        else nn.Linear(lstm_dim, out_dim)
    )

def gru(in_dim, gru_dim, out_dim, act=None):
    """
    GRU with LayerNorm and Mish activations.
    """
    return nn.Sequential(
        GRU(in_dim, gru_dim),
        NormedLinear(gru_dim, out_dim, act=act)
        if act
        else nn.Linear(gru_dim, out_dim)
    )
...
def enc(cfg, out={}):
    ...
    for k in cfg.obs_shape.keys():
        if k == "state":
            # task_dim is 0 for single-task, so we directly use obs_shape
            in_features = cfg.obs_shape[k][0]
            out[k] = mlp(
                in_features,  # Removed + getattr(cfg, "task_dim", 0)
...
def enc_separate(cfg,latent_dim, obs_shape, out={}):
    """
    Returns a dictionary of encoders for each observation in the dict.
    """
...
def dec(cfg, latent_dim, obs_shape, out={}):
    """
    Returns a dictionary of decoders for each observation in the dict.
    """
    for k in obs_shape.keys():
        if k == "state":
            # task_dim is 0 for single-task, so we directly use obs_shape
            ...
            raise NotImplementedError(
                f"Decoder for observation type {k} not implemented."
            )
    return nn.ModuleDict(out)
