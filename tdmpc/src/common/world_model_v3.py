...
class WorldModel(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.latent_separate:
            self.h_obs_pos_dim = cfg.h_obs_shape["state"][0] - cfg.h_obs_vel_dim
            cfg.h_obs_shape = {"state": (self.h_obs_pos_dim,)}
            print("h_obs_pos_dim:", self.h_obs_pos_dim)
            # self.cfg.h_obs_shape = cfg.h_obs_shape
            ...
            self._dynamics_h = layers.mlp(
                cfg.latent_h_dim + cfg.h_obs_vel_dim,
                4 * [cfg.mlp_dim],
                cfg.latent_h_dim,
                act=layers.SimNorm(cfg),
            )
            ...
            self._h_obs_e = self._h_obs_s + self.h_obs_pos_dim + cfg.h_obs_vel_dim
            self._h_obs_pos_s = 0
            self._h_obs_pos_e = self.h_obs_pos_dim
            self._h_obs_vel_s = self._h_obs_pos_e
            self._h_obs_vel_e = self._h_obs_vel_s + cfg.h_obs_vel_dim
            self._latent_r_s = 0
    ...
    def slice_h_observation(self, h_obs):
        """
        Slices the human observation into position and velocity parts.
        """
        h_obs_pos = h_obs[..., self._h_obs_pos_s:self._h_obs_pos_e]
        h_obs_vel = h_obs[..., self._h_obs_vel_s:
                        self._h_obs_vel_e]
        return h_obs_pos, h_obs_vel
    ...
    def encode(self, obs):
        ...
        if self.cfg.latent_separate:
            ...
            h_obs_pos, h_obs_vel = self.slice_h_observation(h_obs)
            z_h = self._encoder["human"](h_obs_pos)
            # print("h_obs shape:", h_obs.shape)
            return torch.cat([z_r, z_h], dim=-1) , h_obs_vel
        if self.cfg.obs == "rgb" and obs.ndim == 5:
            return torch.stack([self._encoder[self.cfg.obs](o) for o in obs])
        return self._encoder[self.cfg.obs](obs)

    def next(self, z, a, h_obs_vel, shift_net = None):
        """
        ...
        if self.cfg.latent_separate:
            ...
            # print("h_obs_vel shape:", h_obs_vel.shape)
            z_next_h = self._dynamics["human"](torch.cat([z_h, h_obs_vel], dim=-1))
            if shift_net is not None:
                # SIFT operation
                # print(f"Applying SHIFT operation \n{z_next_h}")
                z_next_h = shift_net(torch.cat([z_next_h, a], dim=-1))
                # print(f"↓\n{z_next_h}")
