...
@hydra.main(config_name="hdyn_train_buffer_test_model_parameter.yaml", config_path=cfg_path)
def train(cfg: dict):
    """
    ...
    print("Buffer state_dict keys:", state_dict.keys())
    print("Buffer num_eps:", state_dict["_num_eps"])
    buffer_len = len(buffer)
    logger.info(f"Buffer length: {buffer_len} transitions.")
    logger.info("Buffer loaded.")
    buffers = []
    _tmp_buffer = Buffer(cfg, show_buffer_info=False)
    _eps_td: list = []
    _ep_idx = 0
    episodes = state_dict["_buffer"]["_storage"]["_storage"]["episode"][0]
    epoch = cfg.num_epochs
    epoch_ep = episodes//epoch
    logger.info(f"Buffer length: {buffer_len} transitions.")
    logger.info(f"Buffer:{episodes} epoch: {epoch} epoch_ep:{epoch_ep}")
    logger.info(f"Created {len(buffers)} buffers for fine-tuning.")
    ...
    for i in range(buffer_len):
        raw_td = buffer[i]
        td = TensorDict(
            obs=raw_td["obs"].unsqueeze(0),
            action=raw_td["action"].unsqueeze(0),
            reward=raw_td["reward"].unsqueeze(0),
            terminated=raw_td["terminated"].unsqueeze(0),
            batch_size=(1,),
        )
        _eps_td.append(td)
        if torch.isnan(td["terminated"]):
            _ep_idx = _tmp_buffer.add(torch.cat(_eps_td))
            if (_ep_idx % epoch_ep) == 0:
                _ep_num += _ep_idx
                num_updates = cfg.seed_steps
                for i in range(num_updates):
                    _agent_update_metrics = agent.update(buffer)
                    print(colored(f"train step {_step + 1} / {epoch} update {i+1}/{num_updates}", "blue", attrs=["bold"]), end="\r")
                train_metrics.update(_agent_update_metrics)
                current_train_metrics = {}  # Renamed to avoid conflict
                current_train_metrics.update(
                    common_metrics_fn(_step, _ep_num, _start_time)
                )
