...
def eval_fn(cfg: dict, env, agent, logger_instance: Logger, _step: int, logger) -> dict:
    ...
    ep_rewards, ep_successes, ep_lengths, collisions = [], [], [], []
    for i in range(cfg.eval_episodes):
        ...
        for id in range(cfg.num_envs):
            ...
            obs, done, ep_reward, t = env.reset(render = True,env_id=id), False, 0, 0
            if cfg.save_video:
                logger_instance.video.init(env, enabled=enable)
            while not done:
                print(colored(f"eval step {i} / {cfg.eval_episodes} t : {t}", "green", attrs=["bold"]), end="\r")
            ...
            if not cfg.eval_all_envs or cfg.num_envs == 1:
                ...
                if cfg.save_video:
                    logger_instance.video.save(_step, key=f"videos/eval_video/env{id}")
    return dict(
        episode_reward=np.nanmean(ep_rewards),
        episode_success=np.nanmean(ep_successes),
        episode_length=np.nanmean(ep_lengths),
        episode_collision=np.nanmean(collisions),
    )
...
@hydra.main(config_name="custom_model_parameter.yaml", config_path=cfg_path)
def train(cfg: dict):
    """
    ...
    config = OmegaConf.load(os.path.join(cfg_path, "env" , cfg.env_parameter_file))
    ...
    env_cfg.max_episode_length = math.ceil(env_cfg.episode_length_s / env_cfg.dt)
    ...
    if cfg.latent_separate:
        ...
        logger.info(f"latent_dim: {cfg.latent_dim} h_dim: {cfg.latent_h_dim} r_dim: {cfg.latent_r_dim}")
    else:
        cfg.latent_h_dim = "??"
        cfg.latent_r_dim = "??"
    ...
    env = make_nav_env(cfg, env_cfg, env_id="NavigationEnv-custom", use_dt_sep=True)
    logger.info("obs_shape {} r_obs_shape: {}, h_obs_shape: {}".format(cfg.obs_shape,cfg.r_obs_shape, cfg.h_obs_shape))
    ...
    logger.info(colored("Work dir:", "yellow", attrs=["bold"]) + str(cfg.work_dir))
    agent = TDMPC2(cfg)
    buffer = Buffer(cfg)
    logger_instance = Logger(cfg, tdmpc_type=TDMPC_TYPE)

    # Variables from OnlineTrainer.__init__
    _step: int = 0
    _ep_idx: int = 0
    _start_time: float = time()
    _tds: list = []  # From OnlineTrainer.train
    ...
    while _step <= cfg.steps:
        ...
        if done:
            print("")
            logger.info(colored("done!", "green", attrs=["bold"]))
            if eval_next:
                logger.info(colored(f"Evaluating agent... step:{_step}", "blue", attrs=["bold"]))
                eval_metrics_results = eval_fn(cfg, env, agent, logger_instance, _step, logger)
            ...
            if _step > 0:
                if (
                    ...
                    and last_info["terminated"]
                    and not cfg.episodic
                ):
                    raise ValueError(
                        "Termination detected but you are not in episodic mode. "
                        "Set `episodic=true` to enable support for "
                        "terminations."
                    )
                current_train_metrics = {}  # Renamed to avoid conflict
                current_train_metrics.update(
                    episode_reward=torch.tensor(
                        [td["reward"] for td in _tds[1:]]
                    ).sum(),
                    episode_success=(
                        last_info["success"] if last_info is not None else None
                    ),
                    episode_length=len(
                        _tds
                    ),  # Corrected: should be len(_tds) not len(self._tds)
                    episode_terminated=(
                        last_info["terminated"] if last_info is not None else None
                    ),
                    episode_collision=(
                        last_info["collision"] if last_info is not None else None
                    )
                )
                current_train_metrics.update(
                    common_metrics_fn(_step, _ep_idx, _start_time)
                ...
                # print("train metrics: ",current_train_metrics)
                logger_instance.log(current_train_metrics, "train")
                _ep_idx = buffer.add(torch.cat(_tds))

            obs = env.reset(render = cfg.train_render)
            _tds = [to_td_fn(env, obs)]

        print(colored(f"train step {_step} / {cfg.steps}", "blue", attrs=["bold"]), end="\r")
        # Collect experience
        # if _step > cfg.seed_steps:
        #     action = agent.act(obs, t0=len(_tds) == 1)
        # else:
        #     action = env.rand_act()
        np_action = env.base_controller(normalization=True).T[0]
        # print("np_action:", np_action," ", torch.from_numpy(np_action.astype(np.float32)).clone())
        action = torch.from_numpy(np_action.astype(np.float32)).clone()
        obs, reward, done, info = env.step(action)
        last_info = info
        _tds.append(to_td_fn(env, obs, action, reward, info["terminated"]))
        ...
        _step += 1
    logger_instance.save_buffer(buffer, identifier=f"{cfg.seed}")
