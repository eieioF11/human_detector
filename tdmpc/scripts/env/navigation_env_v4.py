...
class NavigationEnv(gym.Env):
    ...
    def __init__(self, cfg: NavigationEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.cfg.dynamic_obstacle_dim = 5  # [x, cos theta, sin theta, vx, wz]
        self.logger.info(f"dynamic_obstacle_dim: {self.cfg.dynamic_obstacle_dim}")
...
# obs, reward, done, truncated, info
    ...
    def _get_env_state(self):
        ...
        self._dynamic_obstacles, self._dynamic_obstacles_fov = get_dynamic_obstacles_v4(
            self.env,
        ...
        # self.logger.info(f"dynamic_obstacles:\n{self._dynamic_obstacles[:,0:2].flatten()}")
        # self.logger.info(f"dynamic_obstacles:\n{self._dynamic_obstacles[:,2:4]}")
        self._pre_state = self._state
    ...
    def make_observation(
        ...
        dist = target_polar[0]
        cos_theta = np.cos(target_polar[1])
        sin_theta = np.sin(target_polar[1])
        ...
        dopos = dynamic_obstacles[:,0:3].flatten()
        dovel = dynamic_obstacles[:,3:5].flatten()
        # dynamic_obstacles_vec = dynamic_obstacles.flatten()
        return np.concatenate((
            np.array([dist, cos_theta, sin_theta], dtype=np.float32),
            # target_polar.astype(np.float32),
            reduced_scan.astype(np.float32),
            dopos.astype(np.float32),
            dovel.astype(np.float32),
