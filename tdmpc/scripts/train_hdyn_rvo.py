...
from tdmpc2_model_base_v2_hdyn import TDMPC2, TDMPC_TYPE
...
@hydra.main(config_name="hdyn_train_rvo_model_parameter.yaml", config_path=cfg_path)
def train(cfg: dict):
    """
    ...
    zero_action = torch.zeros((env_cfg.action_space), dtype=torch.float32)
    ...
    while _step <= cfg.steps:
        ...
        if done:
            ...
            # obs = env.reset(render=cfg.train_render, state=random_state(x_limit=[10.0, 20.0], y_limit=[5.0, 25.0]), collision_off=True)
            obs = env.reset(render=cfg.train_render, collision_off=True)
